...
const MyCourseCard = ({ course, darkMode = false }) => {
  ...
  return (
    <div
      className={`group relative flex flex-col rounded-xl overflow-hidden border transition-all duration-300 hover:scale-[1.02] hover:z-10 btn-shimmer
      ${darkMode
    ...
    >
      ...
      <div className="relative h-40 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 z-40">
          <span className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-lg backdrop-blur-md shadow-sm ${diffTag.class}`}>
            {diffTag.label}
          </span>
        </div>

        <div className="absolute bottom-3 right-3 z-40">
          <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold shadow-lg shadow-black/20 backdrop-blur-md
            ${isCompleted
              ? "bg-emerald-500 text-white"
              : progress_percentage > 0
                ? "bg-indigo-600 text-white"
                : "bg-slate-500/80 text-white"}`}>
            {isCompleted ? "Xong" : progress_percentage > 0 ? `${progress_percentage}%` : "Mới"}
          </span>
        </div>
        ...
        <div className={`absolute inset-0 z-30 flex items-center justify-center p-4 bg-black/40 backdrop-blur-[2px] transition-all duration-300 ${isHovered ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
          <Link
            to={`/courses/${path_id}`}
            className={`w-full max-w-[120px] flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 shadow-2xl ${isHovered ? "scale-100" : "scale-90"}
              ${isCompleted
      ...
      <div className="p-5 flex flex-col flex-1">
        <h3 className={`text-lg font-semibold leading-tight mb-2 line-clamp-1 transition-colors group-hover:text-indigo-500 ${darkMode ? "text-white" : "text-slate-900"}`}>
          {safeTitle}
        </h3>
        ...
        <div className={`grid grid-cols-2 gap-4 py-3 border-t border-slate-100 dark:border-slate-700/50 mb-3`}>
          <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-lg ${darkMode ? "bg-slate-800 text-indigo-400" : "bg-indigo-50 text-indigo-600"}`}>
              <BookOpen className="w-4 h-4" />
            </div>
            <span className={`text-xs font-semibold tracking-tight ${darkMode ? "text-slate-300" : "text-slate-700"}`}>
              {completed_lessons}/{total_lessons_in_path} bài
            </span>
          </div>
          <div className="flex items-center gap-2">
